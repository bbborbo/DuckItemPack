...
namespace DuckItemPack.Items
{
    class CobaltShield : ItemBase
    {
        ...
        public override string ItemFullDescription => $"Become <style=cIsUtility>immune to ALL knockback</style>, and <style=cIsHealing>increase armor</style> " +
            ...
            $"<style=cIsHealing>{baseStationaryArmor}</style> <style=cStack>(+{stackStationaryArmor} per stack) additional armor</style>.";
        ...
        public override GameObject ItemModel => Main.assetBundle.LoadAsset<GameObject>(Main.modelsPath + "Item/CobaltShield.prefab");

        public override Sprite ItemIcon => Main.assetBundle.LoadAsset<Sprite>(Main.iconsPath + "Item/texIconCobaltShield.png");
        ...
        void CreateBuff()
        {
            cobaltDefense = ScriptableObject.CreateInstance<BuffDef>();
            {
                cobaltDefense.name = "CobaltDefense";
                cobaltDefense.buffColor = new Color(0.15f, 0.4f, 0.9f);
                cobaltDefense.canStack = false;
                cobaltDefense.isDebuff = false;
                cobaltDefense.iconSprite = Main.assetBundle.LoadAsset<Sprite>("Assets/Textures/Icons/Buff/texBuffCobaltShield.png");
            };
